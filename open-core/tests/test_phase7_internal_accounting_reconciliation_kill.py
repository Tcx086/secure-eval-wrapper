...
class AccountingTests(unittest.TestCase):
    def test_reservations_partial_release_and_no_overreserve(self):
        a=PaperAccounting(paper_run_id=run_id(),account_reference="a",balances={"USDT":Decimal("100"),"BTC":Decimal("1")}); s=submission(qty="0.5"); a.reserve(s); self.assertEqual(a.reserved("USDT"),Decimal("51.051"))
        with self.assertRaises(ValueError):a.reserve(submission(client="c2",qty="0.6"))
        v=InternalPaperVenue(initial_balances={"USDT":100,"BTC":1}); v.submit_order(s); v.acknowledge("c1",T0); _,f,_=v.fill("c1",Decimal("0.2"),Decimal("100"),T0); a.apply_fill(f); self.assertEqual(a.reserved("USDT"),Decimal("31.031")); a.release("c1"); self.assertEqual(a.reserved("USDT"),0)
