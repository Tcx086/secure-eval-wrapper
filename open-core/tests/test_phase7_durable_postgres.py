...
class DurablePhase7PostgresTests(unittest.TestCase):
    ...
    def test_concurrent_workers_cannot_jointly_exceed_daily_or_approval_budget(self):
        ...
        self.assertEqual(results.count("accepted"),1);state=self.state(engine);self.assertEqual(state["risk_state"]["daily_submitted_notional"],Decimal("612.00"));self.assertEqual(len(state["dispatches"]),1);self.assertEqual(len([r for r in state["reservations"] if r["state"]=="open"]),1)
