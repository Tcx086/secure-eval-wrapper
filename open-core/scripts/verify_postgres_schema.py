...
REQUIRED_COLUMNS: dict[tuple[str, str], tuple[str, ...]] = {
    ...
    ("execution", "funding_payments"): (
        "funding_payment_id",
        "funding_rate_id",
        "settlement_asset",
        "record_sha256",
    ),
    ("execution", "cash_ledger_entries"): (
...
REQUIRED_INDEXES = (
    ...
    ("execution", "position_snapshots", "uq_phase5_position_snapshots_logical"),
    ("execution", "position_snapshots", "idx_phase5_position_snapshots_source"),
    ("execution", "cash_ledger_entries", "uq_phase5_cash_ledger_logical"),
    ("execution", "cash_ledger_entries", "idx_phase5_cash_ledger_source"),
    ("execution", "fills", "uq_phase5_fill_fee_currency"),
...
REQUIRED_CHECK_CONSTRAINTS = (
    ("backtesting", "backtest_runs", "phase5_backtest_runs_fee_base_check", True),
    ("backtesting", "backtest_runs", "phase5_backtest_runs_complete_check", True),
    ("backtesting", "backtest_runs", "phase5_backtest_runs_implementation_sha256_check", True),
    ("execution", "risk_decisions", "phase5_risk_order_lineage_check", True),
    ("execution", "position_snapshots", "phase5_position_snapshots_kind_check", True),
    ("execution", "position_snapshots", "phase5_position_snapshots_mark_provenance_check", True),
    ("execution", "cash_ledger_entries", "phase5_cash_ledger_sequence_check", True),
    ("execution", "order_intents", "phase5_order_intents_implementation_sha256_check", True),
    ("execution", "orders", "phase5_orders_record_sha256_check", True),
    ("execution", "fills", "phase5_fills_record_sha256_check", True),
    ("execution", "positions", "phase5_positions_record_sha256_check", True),
...
ALLOWED_DATA_MIGRATIONS = {
    ...
    "0010_phase5_second_audit_repairs.sql": (
        r"\bUPDATE\s+backtesting\.backtest_runs\s+AS\s+run\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.positions\s+AS\s+position\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.account_snapshots\s+AS\s+snapshot\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.position_snapshots\s+AS\s+snapshot\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.position_snapshots\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.position_snapshots\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.position_snapshots\s+AS\s+snapshot\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.position_snapshots\s+AS\s+snapshot\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.cash_ledger_entries\s+AS\s+entry\s+SET\b.*?;",
        r"\bUPDATE\s+execution\.risk_decisions\s+AS\s+decision\s+SET\b.*?;",
