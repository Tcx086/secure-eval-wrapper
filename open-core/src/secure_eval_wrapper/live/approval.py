...
class LiveApprovalController:
    def create(self, *, report, configuration, account_snapshot, manifest_hash: str, created_at_utc: datetime, ttl_seconds: int, nonce: str, approving_actor: str, maximum_total_approved_notional: Decimal, exact_confirmation_challenge_hash: str) -> LiveApproval:
        ...
        expected_manifest_hash = manifest_preview_hash(
            live_run_id=report.live_run_id, configuration=configuration,
            credential_reference_hash=report.credential_reference_hash,
            preflight_report_id=report.report_id,
            account_snapshot_hash=account_snapshot.record_hash,
            repository_commit_sha=report.repository_commit_sha,
        )
        if manifest_hash != expected_manifest_hash:
            raise LiveApprovalError("manifest preview does not use the observed repository commit")
        ...
        expected = confirmation_challenge_hash(live_run_id=report.live_run_id, configuration=configuration, account_fingerprint=account_snapshot.account_fingerprint, manifest_hash=manifest_hash, repository_commit_sha=report.repository_commit_sha, nonce=nonce, approving_actor=approving_actor, created_at_utc=created_at_utc, expires_at_utc=expires, maximum_total_approved_notional=maximum_total_approved_notional)
        if exact_confirmation_challenge_hash != expected:
            raise LiveApprovalError("exact confirmation challenge mismatch")
        return LiveApproval(report.live_run_id, configuration.configuration_hash, account_snapshot.account_fingerprint, configuration.provider, configuration.environment, configuration.allowed_instruments, maximum_total_approved_notional, created_at_utc, expires, manifest_hash, report.repository_commit_sha, nonce, approving_actor, expected, report.report_id)
    ...
    def validate(self, approval, *, report, configuration, manifest_hash: str, account_snapshot, at_utc: datetime, requested_notional: Decimal = Decimal(0)):
        ...
        if approval.repository_commit_sha != report.repository_commit_sha:
            raise LiveApprovalError("approval repository identity differs from observed preflight authority")
