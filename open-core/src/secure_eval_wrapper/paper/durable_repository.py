...
class DurablePostgresPaperRepository(_Phase7BaseRepository):
    ...
    def _risk(self,c,a,intent,s,state,positions,balances,reservations,evidence,now):
        ...
        requirement=calculate_reservation(s,maximum_fee_bps=Decimal(str(evidence.get("maximum_fee_bps",10))),maximum_adverse_slippage_bps=Decimal(str(evidence.get("maximum_adverse_slippage_bps",200))))
    ...
    def _reserve_and_enqueue(self,s,requirement,state,now):
        rid=deterministic_paper_uuid("reservation",{"submission":s.submission_id});self._strict("execution.paper_reservations","reservation_id",rid,("paper_run_id","submission_id","client_order_id","currency","original_amount","remaining_amount","original_quantity","remaining_quantity","state","created_at_utc","updated_at_utc","version","economics_sha256","reserve_price","maximum_fee_bps","maximum_adverse_slippage_bps","calculator_version","spent_amount"),(s.paper_run_id,s.submission_id,s.client_order_id,requirement.currency,requirement.amount,requirement.amount,s.quantity,s.quantity,"open",now,now,0,s.economics_sha256,requirement.reserve_price,requirement.maximum_fee_bps,requirement.maximum_adverse_slippage_bps,requirement.calculator_version,Decimal(0)),sha256_payload({"reservation":rid,"amount":requirement.amount,"quantity":s.quantity,"calculator":requirement.calculator_version}));self._reservation_event(rid,s,"reserved",requirement.amount,s.quantity,now,s.submission_id)
    ...
    def complete_dispatch_recovery(self,s,*,recovery_claim_token,at_utc,order=None,evidence_sha256=None,classification="venue_query_evidence"):
        with self.transaction():
            ...
            if order.state is VenueOrderState.FILLED and fill_complete:self._release(s,at_utc,row["dispatch_id"],event="consumed")
            if order.state in (VenueOrderState.REJECTED,VenueOrderState.CANCELLED,VenueOrderState.EXPIRED) or (order.state is VenueOrderState.FILLED and fill_complete):self._close_open_once(s,at_utc,row["dispatch_id"],row["recovery_claimed_by"])
    ...
    def complete_dispatch(self,s,*,claim_token,outcome,at_utc,order=None,classification=None,evidence_sha256=None,worker_id=None,fail_at=None):
        ...
        with self.transaction():
            ...
            if state in ("partially_filled","filled") and not fill_complete:state="pending_recovery";self._execute("UPDATE execution.paper_order_submissions SET state=%s,record_sha256=%s WHERE submission_id=%s",(state,sha256_payload({"submission":s.submission_id,"state":state,"missing_fill_evidence":True}),s.submission_id))
    ...
    def complete_cancel_recovery(self,s,*,recovery_claim_token,at_utc,order=None,evidence_sha256=None):
        with self.transaction():
            ...
            if terminal=="filled" and not self._fill_application_complete(s.submission_id,order.cumulative_filled_quantity):
                self._execute("UPDATE execution.paper_cancel_outbox SET state='cancel_unknown',updated_at_utc=%s,record_sha256=%s WHERE cancel_id=%s",(at_utc,sha256_payload({"cancel":row["cancel_id"],"recovery":"filled_without_complete_fill_evidence"}),row["cancel_id"]));self._execute("UPDATE execution.paper_order_submissions SET state='pending_recovery',record_sha256=%s WHERE submission_id=%s",(sha256_payload({"submission":s.submission_id,"state":"pending_recovery","missing_fill_evidence":True}),s.submission_id));self.record_recovery(PaperRecoveryRecord(s.paper_run_id,s.submission_id,row["recovery_claimed_at_utc"],at_utc,RecoveryStatus.PAUSED,"recover_cancel_by_original_client_order_id","FILLED evidence lacked complete fills and fees; reservation remains open",(row["cancel_id"],)));return False
            if terminal is None:
    ...
    def persist_order_observation(self,submission,order,*,observed_at_utc,source,query_id=None,evidence_sha256=None,internal_venue_event_id=None):
        ...
        with self.transaction():
            ...
            if prior is not None:
                ...
                if bool(prior["terminal"]) and str(prior["authority_state"])!=mapping[order.state]:raise Phase7ConflictError("terminal order observation regressed to a different state")
            fill_row=self._fetchone("SELECT COALESCE(sum(quantity),0) applied_quantity,bool_and(accounting_applied) accounting_complete FROM execution.paper_fills WHERE submission_id=%s",(submission.submission_id,));applied=Decimal(str(fill_row["applied_quantity"] or 0));fill_complete=applied==order.cumulative_filled_quantity and bool(fill_row["accounting_complete"] if fill_row["accounting_complete"] is not None else order.cumulative_filled_quantity==0)
            authority=mapping[order.state];blocked=None
            if order.state in (VenueOrderState.PARTIALLY_FILLED,VenueOrderState.FILLED) and not fill_complete:authority="pending_recovery";blocked="missing_complete_fill_fee_accounting_evidence"
            terminal=authority in ("filled","cancelled","rejected","expired")
            if prior is not None and prior["latest_observation_id"]==observation_id and str(prior["authority_state"])==authority and bool(prior["fill_application_complete"])==fill_complete:return False
            version=0 if prior is None else int(prior["version"])+1;projection_hash=sha256_payload({"submission":submission.submission_id,"observation":observation_id,"authority":authority,"filled":order.cumulative_filled_quantity,"complete":fill_complete,"version":version})
            if prior is None:self._execute("INSERT INTO execution.paper_order_projections (submission_id,paper_run_id,client_order_id,venue_order_id,latest_observation_id,venue_sequence,observed_state,authority_state,cumulative_filled_quantity,average_fill_price,terminal,fill_application_complete,blocked_reason,version,updated_at_utc,economics_sha256,record_sha256) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(submission.submission_id,submission.paper_run_id,submission.client_order_id,order.venue_order_id,observation_id,order.venue_sequence,order.state.value,authority,order.cumulative_filled_quantity,order.average_fill_price,terminal,fill_complete,blocked,version,observed_at_utc,order.economics_sha256,projection_hash))
            else:self._execute("UPDATE execution.paper_order_projections SET latest_observation_id=%s,venue_sequence=%s,observed_state=%s,authority_state=%s,cumulative_filled_quantity=%s,average_fill_price=%s,terminal=%s,fill_application_complete=%s,blocked_reason=%s,version=%s,updated_at_utc=%s,record_sha256=%s WHERE submission_id=%s",(observation_id,order.venue_sequence,order.state.value,authority,order.cumulative_filled_quantity,order.average_fill_price,terminal,fill_complete,blocked,version,observed_at_utc,projection_hash,submission.submission_id))
            self.record_order(order)
            self._execute("UPDATE execution.paper_order_submissions SET state=%s,record_sha256=%s WHERE submission_id=%s",(authority,sha256_payload({"submission":submission.submission_id,"state":authority,"observation":observation_id}),submission.submission_id))
            if authority in ("cancelled","rejected","expired"):self._release(submission,observed_at_utc,observation_id);self._close_open_once(submission,observed_at_utc,observation_id)
            if authority=="filled":
    ...
    def reconciliation_authority_checks(self,paper_run_id,*,accounting_reservations,venue_reservations,venue_sequence):
        ...
        projections=self._fetchall("SELECT p.submission_id,p.latest_observation_id,p.fill_application_complete,o.venue_order_observation_id FROM execution.paper_order_projections p LEFT JOIN execution.paper_venue_order_observations o ON o.venue_order_observation_id=p.latest_observation_id WHERE p.paper_run_id=%s",(paper_run_id,))
        for row in projections:
            if row["venue_order_observation_id"] is None:checks.append({"type":"latest_observation_mismatch","identity":str(row["submission_id"]),"local":str(row["latest_observation_id"]),"venue":None,"explanation":"latest order projection lacks its append-only observation"})
            if not bool(row["fill_application_complete"]):checks.append({"type":"fill_application_incomplete","identity":str(row["submission_id"]),"local":False,"venue":True,"explanation":"venue cumulative fill is not fully represented by applied fill/accounting rows"})
        return tuple(checks)
