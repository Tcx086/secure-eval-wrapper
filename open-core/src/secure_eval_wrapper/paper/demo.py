...
def run_internal_demo(*,persist_repository=None):
    ...
    clock.advance(); _,first_fill,_=venue.fill(_client(buy,run_id),Decimal("0.4"),Decimal("100"),clock(),venue_fill_id="demo-fill-1"); engine.poll()
    ...
    clock.advance(); _,second_fill,_=venue.fill(_client(buy,run_id),Decimal("0.6"),Decimal("100"),clock(),venue_fill_id="demo-fill-2"); engine.poll()
    if persist_repository:
    ...
    summary={"paper_run_id":str(run_id),"provider":config.provider.value,"environment":config.environment.value,"preflight_status":report.status.value,"approval_status":approval.state.value,"order_counts":{"submitted":len(broker.submissions),"venue":len(venue._orders)},"fill_counts":{"confirmed":len(broker.local_fills),"persisted":len(persisted_fills) if persist_repository else 0},"reconciliation_status":final_reconciliation.status.value,"reconciliation_difference_count":len(final_differences),"kill_switch_state":kill_controller.current.state.value,"final_balances":{k:format(v,"f") for k,v in sorted(accounting.balances.items())},"final_positions":{p.series_identity.canonical_symbol:{"quantity":format(p.quantity,"f"),"average_entry_price":None if p.average_entry_price is None else format(p.average_entry_price,"f")} for p in accounting.positions.values()},"unknown_submission_recovery":recovery.status.value,"cancel_intent_count":len(cancel_intents),"persistence_status":"postgresql" if persist_repository else "disabled","live_mode":False,"external_network":False}
    if persist_repository:
