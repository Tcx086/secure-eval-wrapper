...
class InternalPaperVenue(PaperVenue):
    def __init__(self,*,account_reference="public-internal-paper",initial_balances=None,fee_bps=Decimal("10"),faults=()):
        self.account_reference=account_reference; self.fee_bps=fee_bps; self._balances={k.upper():Decimal(v) for k,v in (initial_balances or {"USDT":Decimal("10000")}).items()}; self._positions={}; self._reservations={}; self._orders={}; self._venue_ids={}; self._fills={}; self._events=[]; self._sequence=0; self._faults=tuple(faults); self._activated=set(); self._last_snapshot=None
    ...
    def reject(self,client,at,reason="configured_rejection"):
        order=self._orders[client]; self._event("rejected",client,{"reason":reason}); order=replace(order,state=VenueOrderState.REJECTED,updated_at_utc=at,venue_sequence=self._sequence,reject_reason=reason); self._orders[client]=order; self._release_reservation(client); return order
    def expire(self,client,at):
        order=self._orders[client]; self._event("expired",client,{}); order=replace(order,state=VenueOrderState.EXPIRED,updated_at_utc=at,venue_sequence=self._sequence); self._orders[client]=order; self._release_reservation(client); return order
