...
class Portfolio:
    ...
    def __init__(self, *, run_id, initial_cash: Decimal, base_currency: str, config_sha256: str, started_at_utc: datetime) -> None:
        ...
        self.marks: dict[str, tuple[SeriesIdentity, Decimal, datetime]] = {}
        self.ledger: list[CashLedgerEntry] = []
        self.position_snapshots: list[PositionSnapshot] = []
        self.account_snapshots: list[AccountSnapshot] = []
        self.applied_fill_ids: set = set()
        self.total_fees = Decimal(0)
        self.total_funding = Decimal(0)
        self.peak_equity = initial_cash
        self._ledger(LedgerEntryType.INITIAL_CASH, initial_cash, started_at_utc)

    def _ledger(self, entry_type: LedgerEntryType, amount: Decimal, timestamp: datetime, *, series_identity: SeriesIdentity | None = None, fill_id=None, funding_payment_id=None) -> CashLedgerEntry:
        self.cash += amount
        row = CashLedgerEntry(self.run_id, timestamp, entry_type, amount, self.cash, self.base_currency, self.config_sha256, series_identity, fill_id, funding_payment_id)
    ...
    def position(self, identity: SeriesIdentity, mode: AccountingMode, timestamp: datetime) -> PositionState:
        ...
        return existing if existing is not None else empty_position(run_id=self.run_id, series_identity=identity, accounting_mode=mode, timestamp_utc=timestamp, config_sha256=self.config_sha256)
    ...
    def apply_fill(self, fill: Fill) -> PositionSnapshot:
        ...
        if fill.fee_amount != 0:
            self._ledger(LedgerEntryType.FEE, -fill.fee_amount, fill.filled_at_utc, series_identity=fill.series_identity, fill_id=fill.fill_id)
        ...
        return self.snapshot_position(transition.state, fill.filled_at_utc, source_fill_id=fill.fill_id)

    def set_mark(self, identity: SeriesIdentity, price: Decimal, timestamp: datetime) -> PositionSnapshot | None:
        if not price.is_finite() or price <= 0:
            raise ValueError("mark price must be finite and positive")
        self.marks[identity.series_identity_sha256] = (identity, price, timestamp)
        state = self.positions.get(identity.series_identity_sha256)
        return None if state is None else self.snapshot_position(state, timestamp)

    def snapshot_position(self, state: PositionState, timestamp: datetime, *, source_fill_id=None) -> PositionSnapshot:
        ...
        snap = PositionSnapshot(self.run_id, state.position_id, state.series_identity, state.accounting_mode, timestamp, state.quantity, state.average_entry_price, mark, state.realized_pnl, unrealized_pnl(state, mark), age, self.config_sha256, source_fill_id, (state.position_id,))
    ...
    def snapshot_account(self, timestamp: datetime) -> AccountSnapshot:
        ...
        snapshot = AccountSnapshot(self.run_id, timestamp, self.cash, equity, gross, net, realized, unrealized, self.total_fees, self.total_funding, self.config_sha256, stale)
