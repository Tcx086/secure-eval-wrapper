"""Atomic complete-bundle persistence for one monitoring evaluation."""
from __future__ import annotations
from dataclasses import dataclass

class MonitoringBundlePersistenceError(RuntimeError): pass

@dataclass(frozen=True)
class MonitoringBundleSummary:
 check_results:int; snapshots:int; events:int; incidents:int; incident_occurrences:int; latency_samples:int; fix_observations:int

def persist_monitoring_bundle(repository,bundle):
 if repository is None or not hasattr(repository,"transaction"): raise TypeError("monitoring persistence requires an injected transactional PostgreSQL repository")
 try:
 ...
 except Exception as exc: raise MonitoringBundlePersistenceError(f"complete monitoring persistence failed: {exc}") from exc
...
def persist_fix_transition(repository,*,session,at_utc,inbound_messages=(),outbound_messages=(),session_events=(),order_links=(),latency_samples=(),faults=()):
 """Persist accepted messages and the resulting session sequence/state in one transaction."""
 try:
  with repository.transaction():
   repository.record_fix_session(session,at_utc)
   from secure_eval_wrapper.fix.models import FixDirection
   for message in inbound_messages: repository.record_fix_message(session.fix_session_id,FixDirection.INBOUND,message,at_utc,None)
   for message in outbound_messages: repository.record_fix_message(session.fix_session_id,FixDirection.OUTBOUND,message,at_utc,None)
   for event in session_events: repository.record_fix_session_event(session,event)
 ...
 except Exception as exc: raise MonitoringBundlePersistenceError(f"simulated FIX transition persistence failed: {exc}") from exc