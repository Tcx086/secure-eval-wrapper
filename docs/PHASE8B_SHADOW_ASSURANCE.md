...
This mode internally constructs the exact audited `UrlLibHttpTransport` and unauthenticated `OkxPublicProvider`, then permits only `GET /api/v5/public/instruments` followed by `GET /api/v5/market/history-trades`. The production source has no transport constructor argument, test factory, setter, or replaceable transport/provenance slot. Its source-instance identity and provenance capability are generated internally and sealed; provenance copied between source instances is rejected. The trade request is symbols-only (`BTC-USDT`), has an empty instrument tuple and parameter map, uses a fixed five-minute UTC window, `limit<=10`, and `max_pages=1`. It performs no more than those two reads, sends empty headers, accepts no caller-supplied method/path/body/headers, and uses no authenticated account endpoint. The counter increments immediately before each transport send, including a send that raises. Source-issued provenance binds the exact source type and instance, endpoint sequence, actual send count, response hashes, instrument, classification, full payload hash, and failure kind before persistence. Timeouts must be greater than zero and at most ten seconds. A connection or response failure becomes a blocked public-data decision; it cannot be promoted to operational proof. Offline request-contract tests use a separate `fixture_protocol_test` harness and never route a fake transport through production `run_public()`. Once source-issued provenance exists, `run_public()` immediately derives immutable safety facts and durable provenance, then wraps provenance validation, normalization, guarded-policy evaluation, persistence, replay, and conflict handling in a public-safe `ShadowOperationFailure`. The carrier exposes only an allowlisted stage/code plus the authoritative facts and provenance; it does not retain or chain the original exception. The CLI catches it before the generic fail-closed handler, so downstream failures preserve the actual public read count while every write, production-transport, authenticated-endpoint, credential, and production-write count remains zero.
...
Every bundle durably records source classification, ordered endpoint identities, actual read count, response source hashes, source-instance identity, public payload/provenance binding, failure kind, data-provenance hash, and summary hash. Fixture bundles require empty endpoints/hashes, zero reads, and null public provenance. Public success requires the exact two endpoints, two reads, two response hashes, and no failure; unavailable public data requires the actual ordered send prefix, zero to two reads, no more response hashes than completed responses, and a failure kind.

One canonical validator checks the SQL identity columns, exact JSON authority fields, bundle/decision/intent/safety/provenance/summary hash chain, summary consistency, run/config/data/code identity, and all zero-write safety facts. It runs during PostgreSQL repository construction for every existing shadow row, before replay acceptance, before load returns, and therefore before CLI inspect emits evidence. Invalid committed rows fail closed and are never repaired, deleted, overwritten, or shadowed by a second row.

An identical run-ID/payload/provenance replay is idempotent and hash-stable. A different payload or provenance under the same run ID is an explicit conflict. Modified inputs receive new evidence and may bind `parent_input_hash`; previous evidence remains queryable. Fresh-process inspection reloads input, decision, manifest, durable public provenance, summary hash, blockers, and zero-write facts from PostgreSQL.

The verifier executes seven distinct concurrency semantics: identical simultaneous runs; different market snapshots; different synthetic accounts; the same run ID with different payloads; different run IDs with the same payload; replay concurrent with a new run; and a restarted reader concurrent with another write. Each result binds its case ID, expected and observed classification, relevant run IDs/hashes, result hash, and passed flag. The crash matrix still covers all nine points from market normalization through post-commit response loss. Pre-commit failures leave no shadow row. A post-commit response loss leaves exactly one complete recoverable bundle, and retry is an idempotent replay. Preparing or partially finalized evidence is never reported as complete.

## Public assurance evidence

The generated artifact is `docs/evidence/phase8b_shadow_assurance_public.json`. Its keys and key order are fixed by an allowlist, and `evidence_payload_sha256` binds every other field. Its results come from executable deterministic verifier v4, not caller-supplied passed counts. The artifact binds the repository SHA, scenario-catalog hash, runtime-implementation hash, unique per-case results and result hashes for all 54 catalog cases, three restart cases, six replay cases, seven actual in-memory concurrency cases, and all nine crash points. Validation reruns the verifier and rejects count, self-hash, verifier-hash, missing-case, duplicate-case, fake-success, and repository-SHA tampering.
