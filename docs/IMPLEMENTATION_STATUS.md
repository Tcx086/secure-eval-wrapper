...
### Phase 8B: explicit authenticated read-only proof (implementation and operator bootstrap independently audited and accepted; real proof unexecuted and authorization NO; Phase 8C not started)
...
- [x] Implement the dedicated `secure-eval-live-bootstrap` inspect/plan/initialize/verify workflow with pinned immutable migration hashes, exact repository/database/plan identity, an explicit read-only confirmation, the fixed conservative BTC-USDT Spot configuration factory, typed atomic PostgreSQL persistence, public-safe verification, offline attack coverage, and isolated PostgreSQL tests without accessing credentials, OKX, or the existing operator database before independent audit.
